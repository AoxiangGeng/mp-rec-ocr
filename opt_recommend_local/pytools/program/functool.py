...
def retries(max_tries, delay=1, backoff=2, exceptions=(Exception,), ignore_exceptions=None, hook=None):
    ...
    def default_hook(func, tries_remaining, exception, delay):
        ...
        """
        logging.warn("%s: Caught '%s: %s', %d tries remaining, sleeping for %s seconds",
                func.__name__, type(exception).__name__, exception, tries_remaining, delay,
                # 首次输出异常栈, 方便调试时追查问题
                exc_info=tries_remaining == max_tries - 1
                )
                   ...
                   return func(*args, **kwargs)
                    ...
                    if tries_remaining > 0:
                        ...
                        with start_child_span(span, 'sleep',
                                tags=dict(component='functool.retries',
                                    tries=str(max_tries - tries_remaining),
                                    delay=str(mydelay)),
                                ):
                            time.sleep(mydelay)
